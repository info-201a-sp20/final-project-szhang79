...
### Summary Information 
  In our Movies dataset, there are `r number_of_cols` columns. The character types for each column are: `r char_types`. There are `r number_of_rows` rows. These are 10 unique values from the movie names column: `r example_movies_q`. Understanding these characteristics of our dataset gives us a better understanding of the size of our dataset, what types of values we are working with, and the movies that we are analyzing.
