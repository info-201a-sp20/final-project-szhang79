...
budget_vs_income <- function(data, years_input, size_input) {
  ...
  scatter_plot <- ggplot(data = all_data) +
    geom_point(mapping = aes(x = budget_values,
                                    ...
                                    text = paste(
                                      sep = "<br>",
                                      paste("Year:", years),
                                      paste0("Average Budget: $",
                                             round(budget_values / 1000000, 2),
                                             " mil"),
                                      paste0("Average Gross Income: $",
                                             round(gross_values / 1000000, 2),
                                             " mil")
                                    )
                                    ),
  ...
  scatter_plotly <- ggplotly(scatter_plot, width = 820, height = 420,
                             tooltip = "text") %>%
    layout(legend = list(
      x = 100,
      y = 0.5,
